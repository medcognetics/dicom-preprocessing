    ...
    let frames: Option<Vec<usize>> = frames
        .map(|f| {
            ...
            if let Some(first_path) = paths.first() {
                ...
                let frame_count =
                    FrameCount::try_from(&mut decoder).map_err(|_| PyRuntimeError::new_err("Failed to get frame count"))?;
                let frame_count: usize = frame_count.into();
...
impl TiffBatchIterator {
    ...
    fn __next__<'py>(mut slf: PyRefMut<'py, Self>, py: Python<'py>) -> Option<Bound<'py, PyList>> {
...
pub(crate) fn register_submodule<'py>(_py: Python<'py>, m: &Bound<'py, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(get_frame_count, m)?)?;
    m.add_function(wrap_pyfunction!(load_tiff_u8, m)?)?;
    m.add_function(wrap_pyfunction!(load_tiff_u16, m)?)?;
    m.add_function(wrap_pyfunction!(load_tiff_f32, m)?)?;
    m.add_function(wrap_pyfunction!(load_tiff_f32_batched, m)?)?;
    Ok(())
}
